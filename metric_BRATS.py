...
exp_name = 'BraSyn_unified_3_to_1_fold1_finetune'
...
def process_fold_modality(args):
    ...
    eval_metric_list = [ 'psnr_full', 'psnr_tumor', 'psnr_non_tumor', 'ssim_full', 'ssim_tumor', 'ssim_non_tumor','MSE_full', 'MSE_tumor', 'MSE_non_tumor']
    # if os.path.exists(f'./checkpoints/{exp_name}/results/fold{fold_num}/{missing_modality}_metrics.txt'):
    #     raise FileExistsError(f"파일 './checkpoints/{exp_name}/results/fold{fold_num}/{missing_modality}/metrics.txt'가 이미 존재합니다.")
    
    if not os.path.isfile(f'./checkpoints/{exp_name}/results/fold{fold_num}/{missing_modality}_metrics.txt'): # if file doesn't exist
        f = open(os.path.join(f'./checkpoints/{exp_name}/results/fold{fold_num}/{missing_modality}_metrics.txt'), "a")
        f.write(f'case_ids')
    ...
    image_folder = os.path.join(data_dir, 'ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData')
    ...
    f = open(os.path.join(f'./checkpoints/{exp_name}/results/fold{fold_num}/{missing_modality}_metrics.txt'), "a")
    f.write(f'case_avg\t{np.mean(psnr_full_list)}\t{np.mean(psnr_tumor_list)}\t{np.mean(psnr_non_tumor_list)}\t{np.mean(ssim_full_list)}\t{np.mean(ssim_tumor_list)}\t{np.mean(ssim_non_tumor_list)}\t{np.mean(mse_full_list)}\t{np.mean(mse_non_tumor_list)}\t{np.mean(mse_tumor_list)}')
    f.write('\n')
    f.close()
...
if __name__ == "__main__":
    ...
    for fold, modality in fold_modality_list:
