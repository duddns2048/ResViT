...
def define_G(input_nc, output_nc, ngf, which_model_netG,vit_name,img_size,pre_trained_path, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[],pre_trained_trans=True,pre_trained_resnet=0, opt=None):
    netG = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm) # NORM = batch
    if use_gpu:
    ...
    elif which_model_netG == 'res_cnn': # pretrain |  vit_name = Res-ViT-B_16
    ...
    elif which_model_netG == 'resvit': # finetune | vit_name = Res-ViT-B_16
        print(vit_name)
        config_vit = residual_transformers.CONFIGS[vit_name]
        netG = residual_transformers.ResViT(config_vit, input_dim = input_nc,img_size=img_size, output_dim=output_nc, vis=False)
        if opt.phase == 'test':
            netG.load_state_dict(torch.load(pre_trained_path))
        else:
            if pre_trained_resnet:
                pre_trained_model = residual_transformers.Res_CNN(residual_transformers.CONFIGS[vit_name], input_dim= input_nc, img_size=img_size, output_dim=output_nc, vis=False)
...
def define_D(input_nc, ndf, which_model_netD,vit_name,img_size,
    ...
    if use_gpu:
        assert(torch.cuda.is_available())
    if which_model_netD == 'basic': # default
