...
class ResViT_model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(4 + 1, opt.ndf, # input 4 source modality +1 target modality
    ...
    def backward_D(self):
        ...
        fake_AB = torch.cat((self.real_A, self.fake_B[:,self.avail_fn.index(0):self.avail_fn.index(0)+1,:,:]), 1)
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False) 

        # Real
        real_AB = torch.cat((self.real_A, self.real_A[:,self.avail_fn.index(0):self.avail_fn.index(0)+1,:,:]), 1)
