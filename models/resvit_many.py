...
class ResViT_model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(4+1, opt.ndf,
    ...
    def set_input(self, input):
        ...
        self.image_paths = input['src1_paths']

    def forward(self):
        self.real_A = Variable(self.input_A)
        self.fake_B= self.netG(self.real_A[:,0:self.opt.input_nc-1,:,:])
    ...
    def backward_D(self):
        ...
        fake_AB = self.fake_AB_pool.query(torch.cat((self.real_A[:,0:self.opt.input_nc-1,:,:], self.fake_B), 1).data)
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False) #
        # Real
        real_AB = torch.cat((self.real_A[:,0:self.opt.input_nc-1,:,:], self.real_B), 1)
    ...
    def backward_G(self):
        ...
        fake_AB = torch.cat((self.real_A[:,0:self.opt.input_nc-1,:,:], self.fake_B), 1)
