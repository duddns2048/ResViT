...
DEFAULT_CUTOFF = 0.001, 0.999
STANDARD_RANGE = 0, 100
TypeLandmarks = Union[TypePath, Dict[str, Union[TypePath, np.ndarray]]]



class CustomHistogramStandardization(NormalizationTransform):
    ...
    def apply_normalization(
        ...
        normalized, linear_mapping = _normalize(image.data, landmarks, mask=mask.numpy())
        image.set_data(normalized)
        image.linear_mapping = linear_mapping  # Save the linear mapping parameters for denormalization
...
def _normalize(
    ...
    linear_mapping = {'slopes': affine_map[0], 'intercepts': affine_map[1], 'range_perc': range_perc}
       
    return new_img, linear_mapping

def denormalize(
    tensor: torch.Tensor,
    linear_mapping: Dict[str, np.ndarray],
    mask: Optional[np.ndarray] = None,
) -> torch.Tensor:
    slopes = linear_mapping['slopes']
    intercepts = linear_mapping['intercepts']
    range_perc = linear_mapping['range_perc']
    ...
    bin_id = np.digitize(data, range_perc[1:-1], right=False)
    lin_img = slopes[bin_id]
    aff_img = intercepts[bin_id]
    orig_img = (data - aff_img) / lin_img
    orig_img = orig_img.reshape(shape)
    orig_img = orig_img.astype(np.float32)
    orig_img = torch.as_tensor(orig_img)
    return orig_img
